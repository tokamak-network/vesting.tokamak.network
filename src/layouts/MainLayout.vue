<template>
  <div v-if="tokenInformation !== undefined" class="main-layout">
    ...
    <div v-if="activeTab === 'MarketingTON'" class="mton">
      ...
      <button :disabled="true" class="release-button" :class="{ 'not-allowed': parseFloat(tokenInformation['totalBalance'])===0 }" :style="{background: color}" @click="parseFloat(tokenInformation['approvedAmount'])===0?mtonApprove():mtonSwap()">{{ parseFloat(tokenInformation['approvedAmount'])===0?'Approve':'Swap' }}</button>
